Mbuni MMS Gateway, with some new features and restoration work.
These features have been added:
- Support for the receiving of PDP context data on the port specified by
  recvpdp-port in the MMSC config options. The data passed on the
  UDP port should be formatted as: 
  ```
  pdp_add,(IPv4),(MSISDN)
  ```
  for additions, or 
  ...
  pdp_del,(IPv4),(MSISDN)
  ```
  for deletions. Currently
  only IPv4 addresses are supported. The PDP Context table has a default
  size of 65535 entries, which can be increased by modifying PDPSIZE in
  mmsc/mms_recvpdp.c. This feature makes integration with cellular
